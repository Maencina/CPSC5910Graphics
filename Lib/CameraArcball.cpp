...
void CameraAB::Set(int scrnX, int scrnY, int scrnW, int scrnH) {
    ...
    modelview = Translate(tran)*rot;
    fullview = persp*modelview;
    float minS = scrnW < scrnH? scrnW : scrnH;
    arcball.Set(&this->rot, vec2(scrnX+scrnW/2, scrnY+scrnH/2), minS/2-50);
};
...
void CameraAB::Resize(int width, int height) {
    ...
    persp = Perspective(fov, aspectRatio, nearDist, farDist);
    fullview = persp*modelview;
    arcball.Set(&rot, vec2(width, height)/2, (width < height? width : height)/2-50);
}
...
void CameraAB::MouseWheel(int direction, bool shift) {
//  if (shift)
